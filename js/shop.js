...
function buy(id) {
  ...
  for (product of products) {
    if (product.id == id) {
      ...
      value++;
      COUNT_PRODUCT.textContent = value;
...
function cleanCart() {
  ...
  COUNT_PRODUCT.textContent = 0;
  document.getElementById("cart_list").textContent = "";
...
function generateCart() {
  ...
  document.getElementById("cart_list").textContent = "";
  for (let i = 0; i < cartList.length; i++) {
    const EXIST = cart.indexOf(cartList[i]);
    if (EXIST === -1) {
      ...
      cart[EXIST].quantity++;
      cart[EXIST].subtotal = cart[EXIST].price + cart[EXIST].quantity;
    }
  }
  console.log(cart);
  applyPromotionsCart();
